...
#define START_TIMER_HK {NSTimeInterval start = [NSDate timeIntervalSinceReferenceDate];  
#define END_TIMER_HK(msg)  NSTimeInterval stop = [NSDate timeIntervalSinceReferenceDate]; \
debugLog([NSString stringWithFormat:@"%@ Time = %f", msg, stop-start])};  

#else

#define START_TIMER_HK
#define END_TIMER_HK(msg)  
