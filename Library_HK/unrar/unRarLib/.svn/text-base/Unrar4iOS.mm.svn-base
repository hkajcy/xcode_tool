...
-(BOOL) unrarOpenFile:(NSString*)rarFile {
	...
	return [self _unrarOpenFile:self.filename mode:RAR_OM_LIST];
}
...
-(NSArray *) unrarListFiles 
{
    ...
    if (![self _unrarOpenFile:filename mode:RAR_OM_LIST])
    {
...
-(NSData *) extractStream:(NSString *)aFile {
	...
	if (![self _unrarOpenFile:filename mode:RAR_OM_EXTRACT])
    {
        [self _unrarCloseFile];
        return nil;
    }
