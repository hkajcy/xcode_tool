...
OEBCSSStyle* StyleSheetTable::createOEBCSSStyle(NSString *tag, NSString *aClass) {
    if (nil == tag || nil == aClass) {
        return nil;
    }
    shared_ptr<ZLTextStyleEntry> entry = control([tag UTF8String], [aClass UTF8String]);
    if (entry.isNull()) {
        return nil;
    }
    return StyleSheetTable::createOEBCSSStyle(*entry);
}
...
OEBCSSStyle *StyleSheetTable::createOEBCSSStyle(ZLTextStyleEntry &entry) {
    // 
    OEBCSSStyle *style = [[[OEBCSSStyle alloc] init] autorelease];
    // 文本对齐方式。暂不支持ALIGN_JUSTIFY两端对齐方式。
    if (entry.alignmentTypeSupported()) {
        switch (entry.alignmentType()) {
            case ALIGN_RIGHT:
                [style setTextAlign:OEBTextAlignmentRight];
                break;
            case ALIGN_CENTER:
                [style setTextAlign:OEBTextAlignmentCenter];
                break;
            case ALIGN_LEFT:
            default:
                [style setTextAlign:OEBTextAlignmentLeft];
                break;
        }
    } else {
        [style setTextAlign:OEBTextAlignmentNone];
    }
    
    OEBFont *oebFont = [[OEBFont alloc] init];
    style.fontStyle = oebFont;
    [oebFont release];
    
    // 字体风格
    OEBFontStyle fontStyle = OEBFontStyleNone;
    if (entry.supportedFontModifier()) {
        fontStyle = OEBFontStyleNormal;
        unsigned char value = entry.fontModifier();
        if (FONT_MODIFIER_BOLD == (value & FONT_MODIFIER_BOLD) 
            && FONT_MODIFIER_ITALIC == (value & FONT_MODIFIER_ITALIC)) {
            fontStyle = OEBFontStyleBoldItalic;
        } else {
            if (FONT_MODIFIER_BOLD == (value & FONT_MODIFIER_BOLD)) {
                fontStyle = OEBFontStyleBold;
            }
            if (FONT_MODIFIER_ITALIC == (value & FONT_MODIFIER_ITALIC)) {
                fontStyle = OEBFontStyleItalic;
            }
        }
    }
    oebFont.style = fontStyle;
    
    NSString *fontName = nil;
    if (entry.fontFamilySupported()) {
        fontName = [OEBCSSStyle nearnessFontNameForFamilyName:
                    [NSString stringWithUTF8String:entry.fontFamily().data()] withFontStyle:fontStyle];
    }
    oebFont.fontName = fontName;
    
    // 字体大小
    CGFloat fontSize = -MAXFLOAT;
    if (entry.fontSizeSupported()) {
        switch (entry.fontSizeMag()) {
            case -3:    // xx-small
                fontSize = kCSSFontSize_XXSmall;
                break;
            case -2:    // x-small
    ...
    oebFont.fontSize = fontSize;
    
    [style setTextFont:[oebFont getUIFont]];
    
    
    // 文本首行缩进
    if (entry.lengthSupported(ZLTextStyleEntry::LENGTH_FIRST_LINE_INDENT_DELTA)) 
    {
        ZLTextStyleEntry::LengthType lengthType = entry.length(ZLTextStyleEntry::LENGTH_FIRST_LINE_INDENT_DELTA);
        if (lengthType.Size > 0) {
            style.textIndent = YES;
        }
    }
    
    if (entry.lengthSupported(ZLTextStyleEntry::LENGTH_LEFT_INDENT)) 
    {
        ZLTextStyleEntry::LengthType lengthType = entry.length(ZLTextStyleEntry::LENGTH_LEFT_INDENT);
        if (lengthType.Size > 0) {
            style.textIndent = YES;
        }
    }
    ...
    // 字体颜色
    if (entry.textColorSupported()) {
        style.textColor = [NSString stringWithUTF8String:entry.textColor().data()];
    }
    
    return style;
}

OEBCSSStyle* StyleSheetTable::createOEBCSSStyle(const AttributeMap &map) {
    shared_ptr<ZLTextStyleEntry> entry = createControl(map);
    if (entry.isNull()) {
        return nil;
    }
    return createOEBCSSStyle(*entry);
}
...
shared_ptr<ZLTextStyleEntry> StyleSheetTable::createControl(const AttributeMap &styles) {
    ...
    setLength(*entry, ZLTextStyleEntry::LENGTH_LEFT_INDENT, styles, "padding-left");
	setLength(*entry, ZLTextStyleEntry::LENGTH_RIGHT_INDENT, styles, "margin-right");
    setLength(*entry, ZLTextStyleEntry::LENGTH_RIGHT_INDENT, styles, "padding-right");
    ...
    const std::vector<std::string> &color = values(styles, "color");
    std::string colorStr;
    if (!color.empty()) {
        ZLStringUtil::append(colorStr, color);
    }
    if (!colorStr.empty()) {
        entry->setTextColor(colorStr);
    }
