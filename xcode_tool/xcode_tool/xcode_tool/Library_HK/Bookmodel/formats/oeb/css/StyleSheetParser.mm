...
shared_ptr<ZLTextStyleEntry> StyleSheetSingleStyleParser::parseString(const char *text) {
	myReadState = myReadState_ATTRIBUTE_NAME;
...
void StyleSheetParser::parse(const char *fileName) {
    FILE *fb = fopen(fileName, "rb");
	if (fb) {
		char *buffer = new char[1024];
		while (true) {
			int len = fread(buffer, sizeof(char), 1024, fb);
			if (len == 0) {
				break;
			}
			parse(buffer, len);
...
void StyleSheetParser::processControl(const char control) {
	switch (control) {
		case '{':
			myReadState = (myReadState == TAG_NAME) ? myReadState_ATTRIBUTE_NAME : BROKEN;
			break;
		case '}':
		...
		case ';':
			myReadState =
				((myReadState == ATTRIBUTE_VALUE) ||
				 (myReadState == myReadState_ATTRIBUTE_NAME)) ? myReadState_ATTRIBUTE_NAME : BROKEN;
			break;
		case ':':
			myReadState = (myReadState == myReadState_ATTRIBUTE_NAME) ? ATTRIBUTE_VALUE : BROKEN;
			break;
	}
}
...
void StyleSheetParser::processWordWithoutComments(const std::string &word) {	
	switch (myReadState) {
		...
		case myReadState_ATTRIBUTE_NAME:
