...
nsUniversalDetector::nsUniversalDetector(PRUint32 aLanguageFilter)
{
  ...
  mStart = PR_TRUE;
  mDetectedCharset = HK_ENCODING_ENCODING_DEFAULT;
  mGotData = PR_FALSE;
...
nsresult nsUniversalDetector::HandleData(const char* aBuf, PRUint32 aLen)
{
      ...
      switch (aBuf[0])
        ...
        case '\xEF':
          if (('\xBB' == aBuf[1]) && ('\xBF' == aBuf[2]))
            ...
            mDetectedCharset = HK_ENCODING_UTF8;
        break;
        case '\xFE':
          if ('\xFF' == aBuf[1])
            // FE FF  UTF-16, big endian BOM
            mDetectedCharset = HK_ENCODING_UNICODE_BIG_ENDIAN;
        break;
        case '\xFF':
          if ('\xFE' == aBuf[1])
            // FF FE  UTF-16, little endian BOM
            mDetectedCharset = HK_ENCODING_UNICODE_LITTLE_ENDIAN;
        break;
      }  // switch

      if (mDetectedCharset != HK_ENCODING_ENCODING_DEFAULT)
      {
        mDone = PR_TRUE;
        return NS_OK;
      }
  ...
  case eEscAscii:
    ...
    {
      mDone = PR_TRUE;
      mDetectedCharset = mEscCharSetProber->GetCharSetEncoding();
    }
    break;
  case eHighbyte:
    for (i = 0; i < NUM_OF_CHARSET_PROBERS; i++)
    {
      if (mCharSetProbers[i])
      {
        ...
        {
          mDone = PR_TRUE;
          mDetectedCharset = mCharSetProbers[i]->GetCharSetEncoding();
          return NS_OK;
        }
      } 
    }
  ...
  case eHighbyte:
    {
      ...
      if (maxProberConfidence > MINIMUM_THRESHOLD)
        Report(mCharSetProbers[maxProber]->GetCharSetEncoding());
    }
    break;
  case eEscAscii:
    break;
